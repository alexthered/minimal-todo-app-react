...
export default class TodoApp extends React.Component {
	constructor(props) {
		...
		this.state = {
			todos: this.props.dataService.getAllTodos(),
	...
	addTodo = () => {
		if (this._todoInputField.value) {
			this.props.dataService.addTodo(this._todoInputField.value);
			this.setState({todos: this.props.dataService.getAllTodos()});
			this._todoInputField.value = '';
		}
	}

	archiveToggleTodo = e => {
		this.props.dataService.archiveToggleTodo(e.target.dataset.id);
		this.setState({todos: this.props.dataService.getAllTodos()});
	}

	removeTodo = e => {
		this.props.dataService.removeTodo(e.target.dataset.id);
		this.setState({todos: this.props.dataService.getAllTodos()});
	}
